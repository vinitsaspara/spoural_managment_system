...
const ViweSelectedStudent = () => {
   ...
   useEffect(() => {
      const fetchSelectedPlayers = async () => {
         ...
         try {
            const response = await axios.get(`http://localhost:8000/api/v2/registration/${gameId}/getselectedplayers`, { withCredentials: true });
   ...
   const exportToExcel = () => {
      ...
      XLSX.utils.sheet_add_json(worksheet, players.map(player => ({
                        ...
                        <TableBody>
                           {players.map((player) => (
                              <TableRow key={player._id} className="border-t">
                                 <TableCell className="px-6 py-4">{player.student.fullname}</TableCell>
                                 ...
                                 <TableCell className="px-6 py-4 capitalize">{player.status}</TableCell>
                              </TableRow>
                           ))}
                        </TableBody>
                     </Table>
                     {players.length === 0 && <p className="text-center mt-4">No selected students found.</p>}
                  </CardContent>
               </Card>
            )}
         </div>
